(function(){
	...
	$('.nav3 li').on('touchstart',function(){
		setCookie('nav3Count',$(this).index(),28);
		initNav3();
	})
	function initNav3(){
		$('.nav3 li').removeClass('active');
		$('.enterances4 .content').hide();
		$('.nav3 li').eq(getCookie('nav3Count')||0).addClass('active');
		$('.enterances4 .content').eq(getCookie('nav3Count')||0).show();
		if(sessionStorage.getItem('currentScrollT')){
			$('.enterances4').scrollTop(eval(sessionStorage.getItem('currentScrollT3'))[getCookie('nav3Count')]);
		}else{
