	...
	function setSchemeInfo(data){
		...
		for(var i=0;i<data['primeFeesLists'].length;i++){
			...
			if(data['primeFeesLists'][i]['feeTypeNum']=='0'){
				$('#unit').html(arrHyfCurrency[data['primeFeesLists'][i]['currency']]).attr({'currency':data['primeFeesLists'][i]['currency'],'feeCode':data['primeFeesLists'][i]['feeCode']});
				$('.units').html(arrCurrency[data['primeFeesLists'][i]['currency']]);
				for(var key in data['primeFeesLists'][i]){
					if(key.substring(0,4)=='cost'&&data['primeFeesLists'][i][key]!='0'){
						var oLi=createLi('tplLi',json2[key],data[json2[key]]);
						$(oLi).find('input').val(data['primeFeesLists'][i][key]);
						$('#hyf').append($(oLi));
					}
				}
				$('#hyf li:last-child').addClass('cr')
			}else{
				if(data['primeFeesLists'][i]['byOrder']=='1'){
					var oDd=$("<dd class='clearfix Fees'><span class='name fl'>"+data['primeFeesLists'][i]['feeTypeEname']+"</span><span class='val fr'><span class='currencys'>"+arrCurrency[data['primeFeesLists'][i]['currency']]+"</span><input type='number' value='100' disabled/></span></dd>");
					oDd.find('input').val(data['primeFeesLists'][i]['orderPrice']);
				}else{
					var oDd=$("<dd class='clearfix Fees'><span class='name fl'>"+data['primeFeesLists'][i]['feeTypeEname']+"</span><span class='val fr'><span class='currencys'>"+arrCurrency[data['primeFeesLists'][i]['currency']]+"</span><input type='number' value='100' class='v1' disabled/>*<span class='num1'>1</span>+<span class='currencys'>"+arrCurrency[data['primeFeesLists'][i]['currency']]+"</span><input type='number' value='100' class='v2' disabled/>*<span class='num2'>1</span>=<span class='currencys2 currencys'>"+arrCurrency[data['primeFeesLists'][i]['currency']]+"</span><span class='total2'>200</span></span></dd>");
					if($('#num20GP').length){
						oDd.find('.num1').text($('#num20GP').text().substring(5));
					}else{
						oDd.find('.num1').text('0');
					}
					oDd.find('.v1').val(data['primeFeesLists'][i]['cost20gp'])
					if($('#num40GP').length){
					...
					oDd.find('.v2').val(data['primeFeesLists'][i]['cost40gp'])
					toTotal(oDd);
				}
				oDd.find('input').css('background-color','#fff')
				oDd.attr('currenys',data['primeFeesLists'][i]['currency']);
				oDd.attr('byOrder',data['primeFeesLists'][i]['byOrder']);
				oDd.attr('feeCode',data['primeFeesLists'][i]['feeCode']);
				$('#append dt').after(oDd);
//				toTotalAll();
			}
		}
	...
	$('#confirm').on('touchstart',function(){
		...
		updateFees(JSON.stringify(arrFees));
	})
