...
//		console.log(sessionStorage.getItem('datas'))
		if(getCookie('currentCount')==1){
			...
			loadInquirys()
			cancel();
		}else if(getCookie('currentCount')==3){
			loadNodes();
		}
		$('#btnEnquirys').on('touchstart',function(){
		...
		$('#btnInquiry').on('touchstart',function(){
			...
			$('.enterances3').on('scroll',function(){
				switch (getCookie('nav2Count')){
					...
					case '1':
						...
						break;
					case '2':
						currentScrollT2[2]=$('.enterances3').scrollTop();
						break;
				}
...
//		$('.enterances4 .content').each(function(index,ele){
			switch (getCookie('nav3Count')){
					case '0':
						currentScrollT3[0]=$('.enterances4').scrollTop();
						break;
					case '1':
						currentScrollT3[1]=$('.enterances4').scrollTop();
						break;
				}
...
//			console.log($('.enterances2').height()+$('.enterances2').scrollTop())
		...
		$('.enterances3').on('scroll',function(){
//			console.log($('.enterances2').height()+$('.enterances2').scrollTop())
			if(($(this)[0].scrollHeight-600)<($(this).height()+$(this).scrollTop())){
				switch (getCookie('nav2Count')){
					case '0':
						if(scrollBtn21) return;
						iNav2Num1+=pageSize;
						getEnquirys(getCookie('accessToken'),0,iNav2Num1,setInquirys0,0);
						break;
					case '1':
						if(scrollBtn22) return;
						iNav2Num2+=pageSize;
						getEnquirys(getCookie('accessToken'),1,iNav2Num2,setInquirys1,0);
						break;
					case '2':
						if(scrollBtn23) return;
						iNav2Num3+=pageSize;
						getEnquirys(getCookie('accessToken'),2,iNav2Num3,setInquirys2,0);
						break;
				}
				cancel();
			...
			if(sessionStorage.getItem('dataNodes')!=null){
				...
				if(sessionStorage.getItem('currentData3')&&sessionStorage.getItem('currentScrollT3')){
					...
					for(var i=0;i<arr1.length;i++){
						...
						$('.enterances4').scrollTop(arr2[getCookie('nav3Count')]);
				}else{
					findByOrder(orderNum,'0,1',setNewNodes);
					findByOrder(orderNum2,2,setNewNodes2);
				}
		...
		function loadInquirys(){
			if(bSinInquirys) return;
			bSinInquirys=true;
			if(sessionStorage.getItem('datas')!=null){
				switch (getCookie('nav2Count')){
					case '0':
						scrollBtn21=true;
						setInquirys0(eval('('+sessionStorage.getItem('datas')+')'));
						$('.nav2 li:first-child .n').html(eval('('+sessionStorage.getItem('datas')+')').length);
						getEnquirys(getCookie('accessToken'),1,iNav2Num2,setInquirys1,0);
						getEnquirys(getCookie('accessToken'),2,iNav2Num3,setInquirys2,0);
						break;
					case '1':
						scrollBtn22=true;
						setInquirys1(eval('('+sessionStorage.getItem('datas')+')'));
						$('.nav2 li:nth-child(2) .n').html(eval('('+sessionStorage.getItem('datas')+')').length);
						getEnquirys(getCookie('accessToken'),0,iNav2Num1,setInquirys0,0);
						getEnquirys(getCookie('accessToken'),2,iNav2Num3,setInquirys2,0);
						break;
					case '2':
						scrollBtn23=true;
						setInquirys2(eval('('+sessionStorage.getItem('datas')+')'));
						$('.nav2 li:last-child .n').html(eval('('+sessionStorage.getItem('datas')+')').length);
						getEnquirys(getCookie('accessToken'),0,iNav2Num1,setInquirys0,0);
						getEnquirys(getCookie('accessToken'),1,iNav2Num2,setInquirys1,0);
						break;
				}
				...
				if(sessionStorage.getItem('currentData2')&&sessionStorage.getItem('currentScrollT2')){
					...
					iNav2Num1=arr1[0].length-pageSize;
					iNav2Num2=arr1[1].length-pageSize;
					iNav2Num3=arr1[2].length-pageSize;
					setInquirys0(arr1[0])
					setInquirys1(arr1[1])
					setInquirys2(arr1[2])
					currentData2=[[],[],[]];
